...
const emp_api_key = process.env.EMPLODE_API_KEY!
const emp_project_id = process.env.EMPLODE_PROJECT_ID!

async function getDebugUrl(id: string) {
  const response = await fetch(`https://www.browserbase.com/v1/sessions/${id}/debug`, {
    method: "GET",
    headers: {
      "x-bb-api-key": emp_api_key,
        ...
        execute: async ({ query, sessionId }) => {
          try {
            ...
            const browser = await chromium.connectOverCDP(
              `wss://connect.browserbase.com?apiKey=${emp_api_key}&sessionId=${sessionId}`
            );
      ...
      getPageContent: tool({
        ...
        execute: async ({ url, sessionId }) => {
          try {
            ...
            const browser = await chromium.connectOverCDP(
              `wss://connect.browserbase.com?apiKey=${process.env.EMPLODE_API_KEY}&sessionId=${sessionId}`
            );
            const defaultContext = browser.contexts()[0];
            const page = defaultContext.pages()[0];
